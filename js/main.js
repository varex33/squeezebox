jQuery(document).ready(function(){
	var intro = jQuery('.cd-intro-block'),
		projectsContainer = jQuery('.cd-projects-wrapper'),
		...
		projectInfo = jQuery('.cd-projects-wrapper').find('.wp-post'); // Added by Sam
	...
	intro.on('click', function(event) {
		...
		if( intro.hasClass('projects-visible') && !jQuery(event.target).is('a[data-action="show-projects"]') ) {
	...
	projectInfo.click(function(){
		var post_link = jQuery(this).find('span').html();
	...
	function nextSides(slider) {
		...
		if( jQuery('.no-csstransitions').length > 0 ) updateSlider('next', actual, slider, following);
	}
	...
	function prevSides(slider) {
		...
		if( jQuery('.no-csstransitions').length > 0 ) updateSlider('prev', actual, slider);
	}
